...
from app.kudo.service import Service as Kudo # Q: What does "import as" serve to do?
...
@login_required
def create():
    github_repo = GithubRepoSchema().load(json.loads(request.data)) # Q: Where is the request information coming from?
...
def update(repo_id):
    ...
    if kudo_service.update_kudo_with(repo_id, github_repo):
        return json_response(github_repo)
    else:
        return json_response({'error': 'kudo not found'}, 404)
...
def json_response(payload, status=202):
